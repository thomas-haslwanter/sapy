...
def normalPlot() -> None:
    ...
    plt.plot(t, s)
    plt.title(
        "Normal plot: you have to close it to continue\n"
...
def positionOnScreen() -> None:
    ...
    Does not work very reliably, not clear why.
    """
...
def showAndPause() -> None:
    ...
    plt.plot(t, s)
    plt.title("Dont touch! I will proceed automatically.")
...
def plotly_demo() -> None:
    ...
    Paul    180     68
    Mary    165     65"""
    df = pd.read_csv(io.StringIO(in_data), sep=r"\s+")
