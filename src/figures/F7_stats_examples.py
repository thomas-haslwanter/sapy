...
sns.set(context='paper', style='ticks', font_scale=1.7)

# Generate the data
num_samples = 50
bmi = pd.DataFrame({'value':[27.3, 28.8, 26.5, 24.2]},
        index=['Germany', 'USA', 'Austria','China'])
std = 4
samples = {}
np.random.seed(123)
...
plt.hlines(np.mean(samples['Germany']), 0, num_samples, linestyles='--',
        color='b', label='mean-Germany')

plt.plot(np.arange(num_samples, 2*num_samples), samples['Austria'],'ro', ms=3,
        fillstyle='none', label='Austrians')
...
# (weight-loss 1 kg)
weight_loss = 1
samples['Germany2'] = samples['Germany'] - weight_loss + \
        np.random.randn(num_samples)*0.5

fig, axs = plt.subplots(1,2, sharey=True)
axs[0].plot(samples['Germany'], 'o', ms=3, label='before diet')
axs[0].plot(np.arange(num_samples, 2*num_samples), samples['Germany2'],'o',
        fillstyle='none', ms=3, label='after diet')
axs[0].set_xlabel('Subject-Nr')
axs[0].set_ylabel('BMI')
axs[0].legend(loc='upper right')

axs[1].plot(samples['Germany'], 'o', ms=6, label='before diet')
axs[1].plot(samples['Germany2'],'o', ms=6, fillstyle='none',
             label='after diet')
